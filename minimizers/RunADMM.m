...
for k = 1:1:outer_iters
    ...
    if isfield(params,'use_fft') && params.use_fft == true        
        b = fft_Ht_y + gamma*fft2(v_est-u_est);
        A = fft_HtH + gamma;
        x_est = real(ifft2( b./A ));
        x_est = max( min(x_est, 255), 0);
    else % use gradient descent        
        for j = 1:1:inner_iters
            b = Ht_y + gamma*(v_est - u_est);
            A_x_est = BackwardFunc(ForwardFunc(x_est))/(input_sigma^2) + gamma*x_est;
            res = b - A_x_est;
            a_res = BackwardFunc(ForwardFunc(res))/(input_sigma^2) + gamma*res;
    ...
    for j = 1:1:inner_denoiser_iters
        f_v_est1 = Denoiser1(v_est);
        f_v_est2 = Denoiser(v_est, effective_sigma);
        v_est = (gamma*(x_hat + u_est) + lambda*f_v_est1 + beta*f_v_est2)/(lambda + beta + gamma);
    end
